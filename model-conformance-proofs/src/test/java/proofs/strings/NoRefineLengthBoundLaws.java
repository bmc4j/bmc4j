...
/**
 * Length-bound conformance for symbolic strings under string refinement OFF (StringMode.NONE).
 *
 ...
 *   <li>the global bound ({@code @BmcProof(maxStringLength = n)}), which under NONE backs a bare
 *       symbolic ({@code nondetWithoutNull}) string.</li>
 * </ul>
 *
 * <p>Before the StringLengthBytecode transform, anyString under NONE introduced the string as a raw
 ...
 * REFINEMENT, so they pin a mode-agnostic guarantee, not a NONE-only quirk.
 */
class NoRefineLengthBoundLaws {

    // ---- per-call anyString(n): the bound must bind across a SECOND read (NONE) ----
    ...
    // length is always <= 16). Under NONE the backing bound must be the per-call 20, not the global.

    @BmcProof(unwind = 21, stringMode = StringMode.NONE)
    ...
    // ---- global @BmcProof(maxStringLength) backs a bare symbolic string under NONE ----

    @BmcProof(unwind = 3, maxStringLength = 2, stringMode = StringMode.NONE)
