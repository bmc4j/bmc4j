...
// Resource-only jar that bundles the JBMC engine for macOS x64. The binary is
...
// token from ghcr.io/token (fetched here in-JVM). The bottle is :any_skip_relocation,
// so the single Intel (sonoma) bottle runs on the macos-13/Ventura assembly runner too.
// Extraction relies on `tar`, so this module can only be ASSEMBLED on macOS; its
// assembly tasks are gated with `onlyIf` so the build still configures on any OS.
...
val prepareEngine by tasks.registering {
    onlyIf("macOS x64 engine jar can only be assembled on macOS x64") { isThisPlatform }
        ...
        if (!exe.exists() || !models.exists()) {
            ...
            if (!actual.equals(bottleSha256, ignoreCase = true)) {
                throw GradleException(
                    "Checksum mismatch for macOS x64 bottle\n  expected $bottleSha256\n  actual   $actual")
            }
