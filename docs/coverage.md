...
| `Float.compare` / `Double.compare` (JDK total order) | ✅ | **MODELED** (`java.lang.Float`/`java.lang.Double` models). jbmc's *native* `*.compare` is unsound (the equal / -0.0-vs-+0.0 / NaN cases are left UNCONSTRAINED), so bmc4j supplies its own sound total order: `-0.0 < +0.0`, NaN largest (sorts above `+Inf`), `compare(NaN,NaN)==0`. **Modeled BIT-FREE** — `floatToIntBits`/`doubleToLongBits` are themselves UNSOUND under jbmc (a nondet stub: even `floatToIntBits(1.0f) != 0x3F800000`; a symbolic finite float reads back `+Inf`'s pattern), so the JDK's bit-flip algorithm can't be used. Instead the order is derived from the sound primitives: `< > ==`, NaN via `a!=a`, and `-0.0`-vs-`+0.0` via the reciprocal sign (`1.0f/-0.0f == -Inf`, `1.0f/+0.0f == +Inf`). Pinned in `proofs.primitives.FloatDoubleCompareLaws` and differential-tested vs the real JDK in `conformance.ArraysUtilConformanceTest`. `floatToIntBits`/`doubleToLongBits`/parse/format stay **loud** in the Float/Double `@BmcModelTail` |
