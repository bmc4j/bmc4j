...
| `Float.compare` / `Double.compare` (JDK total order) | ❌ | UNSOUND in jbmc: the sign is correct for strictly-ordered finite values, but the **equal / -0.0-vs-+0.0 / NaN** cases are left UNCONSTRAINED (modeled-but-incomplete, not stubbed). **Any model that leans on `*.compare` for the IEEE total order is unsound** — bmc4j ships none: the `float[]`/`double[]` `sort`/`equals`/`compare`/`binarySearch`/`mismatch` Arrays overloads (which need this total order for NaN/-0.0) stay **loud** in the `Arrays` `@BmcModelTail` (reaching one is a member-named UNKNOWN, never a false green). Use the primitive `< > ==` (sound, prior row) for FP ordering, not `*.compare`. **Follow-up (deferred):** bmc4j could *model* `Float.compare`/`Double.compare` itself (JDK total order via `floatToIntBits`/`doubleToLongBits`: -0 < +0, NaN largest, `NaN==NaN`) to unblock the loud `float[]` Arrays `sort`/`equals`/`compare`/`binarySearch` members — that introduces new `Float`/`Double` model classes with their own gate/conformance entries, so it's tracked as future work rather than bundled here |
