...
| `float`/`double` arithmetic (`+ - * /`) + primitive comparison (`== < >`) | ✅ | SOUND and **bit-precise** for both `float` and `double` under jbmc's default `floatbv` IEEE-754 semantics (no flag). Arithmetic identities (`a+0==a`, `a*1==a`), commutativity, and the native `fcmpl`/`dcmpl` comparison laws (`-0.0==0.0`, `NaN!=NaN`, every ordered NaN-comparison false, strict finite ordering) all hold; pinned in `proofs.primitives.FloatLaws` / `DoubleArithmeticLaws`. **Non-associativity** (`(a+b)+c == a+(b+c)`) correctly **REFUTES** — the suite asserts that with `expect=REFUTED`, pinning that jbmc never abstracts FP add to real arithmetic. `double` works but is **discouraged**: double *stringification* is unsound (next row) and symbolic FP is heavy — prefer exact `int`/`long`/`BigDecimal` |
| `float[]`/`double[]` `Arrays` total-order ops (`sort`/`equals`/`binarySearch`/`compare`/`mismatch`) | ✅ | **MODELED** via the IEEE total order (`-0.0 < +0.0`, NaN largest, `compare(NaN,NaN)==0`). jbmc's *native* `Float`/`Double.compare` is unsound (the equal / -0.0-vs-+0.0 / NaN cases are left UNCONSTRAINED) AND `floatToIntBits`/`doubleToLongBits` are a nondet stub (even `floatToIntBits(1.0f) != 0x3F800000`), so the JDK's bit-flip algorithm can't be reproduced. bmc4j instead supplies the order **BIT-FREE** in the `org.bmc4j.models.audit.FpTotalOrder` helper, derived from the sound primitives: `< > ==`, NaN via `a!=a`, and `-0.0`-vs-`+0.0` via the reciprocal sign (`1.0f/-0.0f == -Inf`, `1.0f/+0.0f == +Inf`). The `float[]`/`double[]` `Arrays` `sort`/`equals`/`binarySearch`/`compare`/`mismatch` overloads (full-array + ranged) call this helper and are reclaimed from the loud tail. Pinned in `proofs.primitives.FloatDoubleArraysLaws` and differential-tested vs the real JDK in `conformance.ArraysUtilConformanceTest`. **NB the helper is NOT a `java.lang.Float`/`Double` model** — those classes are reached pervasively (autoboxing `*.valueOf`, `<clinit>`), so putting a bounded FP model in `java.lang` crashed jbmc's solver on unrelated proofs; the helper keeps the total order off every other proof's classpath. So **user-facing `Float.compare`/`Double.compare` are NOT modeled** (they fall to jbmc's unsound native compare) — use the sound primitive `< > ==` for FP ordering, or `Arrays.sort`/`equals` for the total order. `hashCode(float[])`/`hashCode(double[])`/`floatToIntBits`/parse/format stay **loud** |
| `Double.toString` / `String.valueOf(double)` / `"" + aDouble` | ❌ | UNSOUND (dtoa unmodeled — rendered result unconstrained; see the strings section). `Float.toString` / `append(float)` ARE sound. Conservatively over-refutes (no false green) |
...
> **Per-member audit.** The exact member-by-member coverage of each modeled JDK class is mechanically
> enforced by four mutually-exclusive per-member audit annotations and **generated** into
> [model-coverage.md](model-coverage.md): `@BmcModelConforms` (modeled, sound under BMC),
> `@BmcUnmodelable` (**loud-if-reached** — genuinely cannot be modeled; a hand-written loud stub diverts
> the reach to a member-named UNKNOWN), `@BmcNotNeeded` (**green-if-reached** — needs no model because
> the unmodeled real/inline bytecode is sound under JBMC, so reaching it is fine), and `@BmcModelTail`
> (the build-synthesized loud exotic remainder). Regenerate via `gradlew -p core
> :bmc-models-conformance:test --tests conformance.ModelCoverageDocsTest -Dbmc.regenerateDocs=true`. The
> rows below stay as a human-readable summary.

| API | Status | Note |
|---|---|---|
| Boxed primitives, `Math` | ✅ | bundled in JBMC `core-models`; the integer methods jbmc stubbed to nondet (`floorDiv`/`floorMod`/`addExact`/`subtractExact`/`multiplyExact`/`negateExact`/`incrementExact`/`decrementExact`/`toIntExact`) are soundly redirected to `BmcMath` via an in-layer bytecode pass (`MathBytecode`), overflow loud. FP `Math`: `abs`/`sqrt`/`pow`/`min`/`max` pass through to the modeled core-models and are **sound over symbolic doubles** (verified). The **transcendentals** `sin`/`cos`/`tan`/`log`/`exp` — and even `floor` — are modeled by the engine but **imprecise on symbolic inputs** (e.g. it admits `sin(0)==1`, `floor(0)` a denormal), so a symbolic-input property over them can spuriously REFUTE; reliable only over **concrete** inputs (see [limits](limits.md)) |
...
| `Map.toSortedMap()` (natural ordering) | ✅ | `MapsKt.toSortedMap(Map)` builds the bounded, natural-ordering `TreeMap` model; @BmcProof (`proofs.kotlincollections.KotlinToSortedMapLaws`). The comparator overload `toSortedMap(Map, Comparator)` is a loud tail stub — the TreeMap model is natural-ordering only |
