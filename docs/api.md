...
| `checkThrows<E> { … }`, `throws<E> { … }` *(Kotlin)* | check a block throws an `E` for every allowed input **and return the caught `E`** — the exception is an ordinary object under BMC, so its *fields* are provable too (`val e = checkThrows<InsufficientFunds> { … }; check(e.shortfall == requested - balance)` — error-path diagnostics, the least-tested code there is; prefer typed fields over `message`). `throws` is the composable boolean form, for *iff*-laws like `check(throws<E> { Ctor(raw) } == (raw !in valid))`. The BMC analogue of `assertFailsWith`; a throw of any *other* type propagates and fails the proof with its own trace. `inline`, so no lambda object / `invokedynamic`; see [`examples/language-kotlin`](../examples/language-kotlin) |
...
| `@BmcProof void p(User u, String s)` | proof-method **parameters** are symbolic inputs — objects, strings, arrays. Kotlin: a non-null-typed parameter is auto-assumed non-null (its kotlinc null-check prologue becomes `assume`), so the proof ranges over what the type system admits; `T?` parameters keep `null`. `bmc { kotlinNullableParams = true }` restores the honest-JVM null domain |
| `@BmcProof(unwind = N, unwindingAssertions = true, maxStringLength = N, timeoutSeconds = N)` | per-proof loop bound (**default `AUTO`** — bmc4j auto-discovers the smallest sufficient bound by climbing to `unwindCap`; a positive `N` pins it, `0` = build default / `-Dbmc.*`) + bound-sufficiency check, symbolic-string length bound, and wall-clock budget (`timeoutSeconds = 0` = no timeout) |
