...
| `Bmc.intFromProperty("KEY")`, `boolFromEnv(...)`, `doubleFromProperty(...)`, `longFrom…`, `stringFrom…` | the **actual** value of env var / system property `KEY` the run was launched with — pinned into the proof (not symbolic); a required-but-unset var fails the proof (see [`examples/integrations`](../examples/integrations)) |
