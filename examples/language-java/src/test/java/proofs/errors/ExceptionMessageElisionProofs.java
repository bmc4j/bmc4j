 ...
 *       poisons it); run it with {@code -Dbmc.elideMessages=off} to see the before.</li>
...
class ExceptionMessageElisionProofs {
    ...
    // proof is tractable and VERIFIES. Without elision (-Dbmc.elideMessages=off) the byte->String
    ...
    // elision in the presence of an observer. (Forcing elision here with elideMessages = ON would make
    ...
    // SOUNDNESS, the inverse: FORCING elision (elideMessages = ON) over the SAME message-observing proof
    ...
    @BmcProof(elideMessages = ElideMessages.ON, expect = Verdict.REFUTED)
