...
/**
 * The contract for [Account]'s pure instance method `project(Int)`, declared test-side from Kotlin in
 * the standard **object-host** shape (a plain `object` with ordinary member `fun` predicates).
 ...
 * normal parameter; `self.balance` reads the receiver field. (Host-kind — `object` — is independent of
 * the target being an instance method on a `class`.)
 *
...
object AccountContract {
    ...
    fun project(amount: Int): Int = error("mirror")

    @ExpectEnforce(Verdict.REFUTED)
    @Requires("nonNegative")
    @Ensures("exceedsBalance")
    fun projectAgain(amount: Int): Int = error("mirror")
    ...
    fun nonNegative(self: Account, amount: Int): Boolean =
            self.balance in 0..1000 && amount in 0..8

    fun atLeastBalance(result: Int, self: Account, amount: Int): Boolean =
            result >= self.balance

    fun exceedsBalance(result: Int, self: Account, amount: Int): Boolean =
            result > self.balance
}
